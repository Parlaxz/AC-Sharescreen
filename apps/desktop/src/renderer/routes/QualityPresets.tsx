      ...
      {effectiveInfo ? (
        <div className="quality-info-block" style={{ marginTop: "0.25rem" }}>
          ...
          <p className="dim" style={{ fontSize: "0.75rem", lineHeight: 1.4 }}>
            {effectiveInfo.requestedBitrate >= 1000 ? `${(effectiveInfo.requestedBitrate / 1000).toFixed(1)} Mbps` : `${Math.round(effectiveInfo.requestedBitrate)} kbps`} &middot; {effectiveInfo.requestedWidth}&times;{effectiveInfo.requestedHeight} @ {effectiveInfo.requestedFps} fps
          ...
          presets.map((p) => (
            <div key={p.id} className="preset-card card">
              ...
              <div className="preset-details" style={{ fontSize: "0.85rem", margin: "0.5rem 0" }}>
                <p>
                  <strong>Video:</strong> {p.settings.video.sendWidth}&times;{p.settings.video.sendHeight} @ {p.settings.video.sendFps} fps &middot; {p.settings.video.videoBitrateKbps >= 1000 ? `${(p.settings.video.videoBitrateKbps / 1000).toFixed(1)} Mbps` : `${Math.round(p.settings.video.videoBitrateKbps)} kbps`} &middot; {p.settings.video.codec}
                </p>
                <p>
                  <strong>Audio:</strong> {p.settings.audio.bitrateKbps >= 1000 ? `${(p.settings.audio.bitrateKbps / 1000).toFixed(1)} Mbps` : `${Math.round(p.settings.audio.bitrateKbps)} kbps`} &middot; {p.settings.audio.channels} &middot; FEC: {p.settings.audio.fec ? "ON" : "OFF"} &middot; DTX: {p.settings.audio.dtx ? "ON" : "OFF"}
                </p>
